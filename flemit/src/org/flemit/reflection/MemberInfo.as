package org.flemit.reflection
{
	...
	public class MemberInfo
	{
		...
		public function MemberInfo(	owner : Type, 
									name : String, 
									fullName : String, 
									visibility : int, 
									isStatic : Boolean, 
		...
		{
			if(!MemberVisibility.contains(visibility))
				throw new ArgumentError('MemberVisibility must be a valid');
			...
			_qname = _visibility == MemberVisibility.PUBLIC 
						? new QualifiedName(new BCNamespace(	_ns, 
																NamespaceKind.PACKAGE_NAMESPACE
																), name) 
						: new QualifiedName(new BCNamespace(	owner.packageName + 
																':' + 
																owner.name, 
																_visibility), name);
